...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (userData && userData.username) {
        ...
        const elRole = document.getElementById('navRole');

        if (elName) elName.textContent = userData.username;
        if (elWelcome) elWelcome.textContent = userData.username;
        if (elAvatar) elAvatar.textContent = userData.username.charAt(0).toUpperCase();
    ...
    window.navigate = (pageId) => {
        ...
        if (elTitle) elTitle.textContent = titles[pageId] || 'Dashboard';
